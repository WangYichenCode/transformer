...
class CNN(nn.Module):
    def __init__(self):
        ...
        self.conv=nn.Sequential(
            ...
            nn.MaxPool2d(kernel_size=2, stride=2),
            CNNlayer(in_channels=24, out_channels=36, kernel_size=3),
            nn.MaxPool2d(kernel_size=2, stride=2),
            nn.Flatten()
                    )
...
class TransCNN(nn.Module):
    def __init__(self, in_feature, dim_model, num_layers, num_heads, seq_length):
        ...
        self.fnn_layer = nn.Sequential(  # 注意力加残差之后直接卷积
            nn.Linear(in_features=43, out_features=256),
            nn.ReLU(),
    ...
    def forward(self, x):
        ...
        cnn_out = self.cnn(fnn_output.permute(1, 0, 2)[:, np.newaxis, :, :])
        return cnn_out


if __name__ == "__main__":
    trans_model = TransCNN(in_feature=43, dim_model=43, num_layers=2, num_heads=1, seq_length=5).to(device)
    cnn_model = CNN()
    ...
    print(out_trans.shape, out_cnn.shape)
